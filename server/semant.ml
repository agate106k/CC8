...
let rec type_dec ast (nest, addr) tenv env =
   match ast with
    ...
    (* 型宣言の処理 *)
    | InitVarDec (t, s, e) ->
     let tenv', env', addr' = type_var_dec (VarDec (t, s)) (nest, addr) tenv env in
     let expected_ty = create_ty t tenv in
     let actual_ty = type_exp e tenv env' in
     if not (ty_eq expected_ty actual_ty) then
       raise (TypeErr (sprintf "Type mismatch in variable initialization for '%s'" s));
     (tenv', env', addr')
