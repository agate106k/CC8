...
and trans_stmt ast nest tenv env = 
                 ...
                 match ast with
                  ...
                  (* | For (v, e1, e2, s) ->
                          ...
                          ^ let (condCode, l1) = trans_cond (CallFunc ("!=", [(VarExp (Var v)); e2])) nest env in let l2 = incLabel() in sprintf "L%d:" l2
                          ^ condCode
                          ^ trans_stmt s nest tenv env
                          ^ trans_stmt (ExpStmt (IncExp(Var v))) nest tenv env
                          ^ sprintf "\tjmp L%d\n" l2
                          ^ sprintf "L%d:\n" l1 *)
                          | For (v, e1, e2, s) ->
                            trans_stmt (Assign(Var v, e1)) nest tenv env
                            ^ let l_start = incLabel() in
                              ...
                              sprintf "L%d:\n" l_start
                            ^ let (condCode, _) = trans_cond (CallFunc ("!=", [(VarExp (Var v)); e2])) nest env in
                              condCode
                            ^ trans_stmt s nest tenv env
